...
/**
   ...
   */(Configuration);var _default=new Promise((async resolve=>{try{const[tinyMCE,pluginMetadata,setupCommands]=await Promise.all([(0,_loader.getTinyMCE)(),(0,_utils.getPluginMetadata)(_common.pluginName,_common.pluginName),(0,_commands.getSetup)()]);tinyMCE.PluginManager.add(_common.pluginName,(editor=>((0,_options.register)(editor),setupCommands(editor),pluginMetadata))),resolve([_common.pluginName,Configuration])}catch(error){console.error("Error initializing TinyMCE plugin:",error)}}));return _exports.default=_default,_exports.default}));
